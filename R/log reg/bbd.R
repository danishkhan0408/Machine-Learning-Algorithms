...
train_len=levels(factor(train$mst))
test_len=levels(factor(test$mst))

if(length(train_len)<length(test_len))
  print("unequal lengths") else
    print("Equal lengths")
...
basemodel = glm(diagnosis ~ stratum +  age + degree + med_check
               + agp1 + agmn + nlv + liv + weight + aglp + mst, 
               binomial(link="logit"),
               data=train)
#schhol valriable is removed, check for erorrs/singularities in school.
...
evals
plot(evals)
abline(h=0.74, v=0.65) # play with these values
